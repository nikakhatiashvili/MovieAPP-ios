        ...
        signUpUseCase.signUp(email: email, password: password) { [self] result in
            ...
            case .success(_):
                NotificationCenter.default.post(
                    name: Notification.Name("SignUpSuccessNotification"), object: nil)
                self.showAlert(
                    title: "successfully signed up", message: "sign up was a success")
                
            case .failure(let message):
                self.showAlert(title: "Sign-up error:", message: "\(String(describing: message.localizedDescription))")
            }
        }
    }
